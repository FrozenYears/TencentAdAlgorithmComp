...
DCN: Deep Cross Network for explicit feature interaction
"""
...
class CrossNetwork(nn.Module):
    """
    DCN Cross Network - 显式特征交叉
    每层: x_{l+1} = x_0 * (w^T * x_l + b) + x_l
    """
    ...
    def __init__(self, input_dim: int, num_layers: int = 3):
        ...
        for _ in range(num_layers):
            self.cross_layers.append(nn.Linear(input_dim, 1, bias=True))
...
class ESMM_DIN(nn.Module):
    ...
    def __init__(
        ...
        # DCN Cross Network (显式特征交叉)
        self.cross_network = CrossNetwork(tower_input_dim, num_layers=3)

        # CTR Tower (DNN + DCN)
        self.ctr_dnn = self._build_mlp(tower_input_dim, hidden_dims, dropout_rate)
        self.ctr_output = nn.Linear(hidden_dims[-1] + tower_input_dim, 1)

        # CVR Tower (DNN + DCN)
        self.cvr_dnn = self._build_mlp(tower_input_dim, hidden_dims, dropout_rate)
        self.cvr_output = nn.Linear(hidden_dims[-1] + tower_input_dim, 1)

        # 保存tower_input_dim供forward使用
        self.tower_input_dim = tower_input_dim
    ...
    def forward(self, batch):
        ...
        # DCN显式特征交叉
        cross_out = self.cross_network(combined)  # [B, tower_input_dim]

        # CTR Tower: DNN + Cross
        ctr_dnn_out = self.ctr_dnn(combined)  # [B, hidden_dims[-1]]
        ctr_combined = torch.cat([ctr_dnn_out, cross_out], dim=-1)
        p_ctr = torch.sigmoid(self.ctr_output(ctr_combined).squeeze(-1))

        # CVR Tower: DNN + Cross
        cvr_dnn_out = self.cvr_dnn(combined)  # [B, hidden_dims[-1]]
        cvr_combined = torch.cat([cvr_dnn_out, cross_out], dim=-1)
        p_cvr = torch.sigmoid(self.cvr_output(cvr_combined).squeeze(-1))
